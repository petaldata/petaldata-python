...
report = petaldata.datasets.stripe.reports.MRRByMonth(invoices,tz='America/Denver')
df = report.to_frame()
print(df)

# date = pd.Timestamp(datetime.now()).floor(freq='D') - pd.offsets.MonthBegin(1) 
# end_time=pd.Timestamp(date)
# report = petaldata.datasets.stripe.reports.NewCustomers(invoices,tz="America/Denver", end_time=end_time)
# df = report.to_frame()
